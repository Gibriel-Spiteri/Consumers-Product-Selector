...
function StockBadge({ qty, isSpecialOrderStock, atpDate, noReorder }: { qty: number | null | undefined; isSpecialOrderStock?: boolean; atpDate?: string | null; noReorder?: boolean }) {
  const stock = (() => {
    ...
    return (
      <span className="inline-flex items-center text-[10px] font-semibold uppercase tracking-wide px-2 py-0.5 rounded-full bg-amber-50 text-amber-600">
        {formatAtpDate(atpDate) ? `EST: ${formatAtpDate(atpDate)}` : "Out of Stock"}
      </span>
    );
  })();
...
function PriceDisplay({ product }: { product: Product }) {
  if (!product.price) return <span className="text-gray-300 font-normal text-sm">—</span>;
        ...
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="border-b border-gray-100">
              ...
              <th className="px-5 py-3.5 text-[11px] font-semibold uppercase tracking-widest text-gray-400 text-right w-[120px]">Price</th>
...
export default function ExpressBathPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch("/api/products/express-bath");
  ...
  if (isLoading) {
    return (
      <div className="max-w-screen-xl mx-auto px-6 py-16 flex flex-col items-center gap-3 text-gray-400">
        ...
        <p className="text-sm">Loading Express Bath items…</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-screen-xl mx-auto px-6 py-8">
      <nav className="flex items-center gap-1.5 text-[13px] text-gray-400 mb-6">
        ...
        <span className="text-blue-600 font-medium">Express Bath</span>
      </nav>

      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-blue-600">Express Bath</h1>
          <p className="text-sm text-gray-500 mt-1">
            {filtered.length} item{filtered.length !== 1 ? "s" : ""} in the Express Bath program
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          >
            <span className={cn("w-1.5 h-1.5 rounded-full", inStockOnly ? "bg-blue-500" : "bg-gray-300")} />
            In Stock Only
          </button>
      ...
      <div className="flex items-center gap-2 mb-6">
        {parentCategories.length > 1 && (
          <>
            ...
            >
              All
              <span className={cn("text-[11px] font-semibold rounded-full px-1.5 py-0.5", activeCategoryId === null ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-400")}>
                {products.length}
              </span>
            </button>
            {parentCategories.map(cat => (
              ...
              >
                {cat.name}
                <span className={cn("text-[11px] font-semibold rounded-full px-1.5 py-0.5", activeCategoryId === cat.id ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-400")}>
                  {cat.count}
                </span>
              </button>
            ))}
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-gray-400">
          ...
          <p className="text-lg font-medium text-gray-500">No Express Bath items</p>
          <p className="text-sm mt-1">There are no products in the Express Bath program right now.</p>
        </div>
      ) : viewMode === "grid" ? (

...
export default function WithoutAttributesProducts() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["withoutAttributesProducts"],
    queryFn: async () => {
      const res = await fetch("/api/products/without-attributes");
  ...
  if (isLoading) {
    return (
      <div className="max-w-screen-xl mx-auto px-6 py-16 flex flex-col items-center gap-3 text-gray-400">
        ...
        <p className="text-sm">Loading products without attributes…</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-screen-xl mx-auto px-6 py-8">
      <nav className="flex items-center gap-1.5 text-[13px] text-gray-400 mb-6">
        ...
        <span className="text-gray-900 font-medium">Products Without Attributes</span>
      </nav>

      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Products Without Attributes</h1>
          <p className="text-sm text-gray-500 mt-1">
            {filtered.length} item{filtered.length !== 1 ? "s" : ""} without any attributes
          </p>
        </div>
